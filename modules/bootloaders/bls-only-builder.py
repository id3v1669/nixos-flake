...
def install_bootloader(args: argparse.Namespace) -> None:
    ...
    (EFI_SYS_MOUNT_POINT / "loader").mkdir(parents=True, exist_ok=True)
    ...
    if extra_files_dir.exists():
        for root, _, files in extra_files_dir.walk(top_down=False):
            ...
            if actual_root.exists() and not list(actual_root.iterdir()):
                actual_root.rmdir()
            root.rmdir()
...
def main() -> None:
    parser = argparse.ArgumentParser(description=f"Update {DISTRO_NAME}-related BLS entries")
